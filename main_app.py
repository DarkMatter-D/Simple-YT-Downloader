...
#a bunch of labels
text_box = customtkinter.CTkLabel(master=main_app,text="Enter Video URL:",font=('Fira Code', 15,'bold'))
text_box.place(x=70,y=30)
error_label = customtkinter.CTkLabel(master=main_app, text="Invalid Url!",text_color="red",font=('Fira Code', 20, 'bold'),anchor="center")
downloading_label = customtkinter.CTkLabel(master=main_app,text="Downloaded!",text_color="white",font=('Fira Code', 20, 'bold'),anchor="center")


#the main download function
def main_download(url,mode):
    try:
        video = YouTube(url,"ANDROID")
        error_label.destroy()
        downloading_label.place(relx=0.5, rely=0.5,x=-80,y=50)
    except RegexMatchError:
        error_label.place(relx=0.5, rely=0.5,x=-80,y=50)
...
video_button.place(x=195,y=150)
